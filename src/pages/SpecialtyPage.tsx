...
const SpecialtyPage = () => {
  ...
  if (!specialty) return <Navigate to="/services" replace />;

  const related = specialtiesData.filter((s) => specialty.relatedSlugs.includes(s.slug));

  return (
    <div>
      {/* ── SEO Meta (title set via document.title for SPA) ── */}
      {(() => { document.title = `${specialty.title} | DD's MaxxPet Clinic Veterinary Centre`; return null; })()}

      {/* ─────────────────── HERO ─────────────────── */}
      <section
        className="inner-page-hero specialty-hero"
        style={{ backgroundImage: `${specialty.heroGradient}, url('${specialty.heroImage}')` }}
      >
        ...
        <div className="container inner-hero-content animate-fade-in">
          <div className="inner-hero-breadcrumb">
            ...
            <span>{specialty.shortTitle}</span>
          </div>

          <div className="inner-hero-icon-badge specialty-hero-icon">
            <span style={{ fontSize: '1.4rem', lineHeight: 1 }}>{specialty.icon}</span>
          </div>

          <div className="specialty-hero-cat">{specialty.category}</div>

          <h1 className="inner-hero-title">{specialty.title}</h1>
          <p className="inner-hero-subtitle">{specialty.tagline}</p>
          ...
          <div className="inner-hero-stats" style={{ marginTop: '2.25rem' }}>
            {specialty.stats.map((s, i) => (
              <>
                ...
                <div className="inner-hero-stat" key={i}>
      ...
      <section className="section specialty-intro-section">
        <div className="container specialty-intro-grid">
          <div className="specialty-intro-text">
            ...
            <h2 className="mt-4">About <span className="gradient-text">{specialty.shortTitle}</span></h2>
          ...
          <div className="specialty-conditions-box">
            <h3 className="specialty-conditions-title">Conditions We Treat</h3>
            <div className="specialty-conditions-list">
              {specialty.conditions.map((c, i) => (
      ...
      <section className="section specialty-procedures-section">
        <div className="container">
          <div className="section-header animate-fade-in">
            ...
            <h2>Procedures &amp; <span className="gradient-text">Treatments</span></h2>
            <p className="text-muted mt-4">A full range of {specialty.shortTitle.toLowerCase()} services under one roof in Faridabad.</p>
          </div>
      ...
      <section className="section specialty-why-section">
        <div className="container">
          ...
          <div className="specialty-why-grid">
            {specialty.whyChooseUs.map((item, i) => (
        ...
        <section className="section specialty-related-section">
          <div className="container">
            <div className="section-header animate-fade-in">
              ...
              <h2>Related <span className="gradient-text">Specialties</span></h2>
            </div>

            <div className="specialty-related-grid">
              {related.map((rel, i) => (
                <Link
                  to={`/specialties/${rel.slug}`}
